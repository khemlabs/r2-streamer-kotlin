...
public final class SMILParser {
	 ...
	 */
	public static String parseAudio(Node element, String href) {
		String audio, clipBegin, clipEnd;
		if (element.getAttributes().containsKey("src")) {
			audio = element.getAttributes().get("src");
		} else {
			return null;
		}
		if (element.getAttributes().containsKey("clipBegin")) {
			clipBegin = element.getAttributes().get("clipBegin");
		} else {
			return null;
		}
		if (element.getAttributes().containsKey("clipEnd")) {
			clipEnd = element.getAttributes().get("clipEnd");
		} else {
			return null;
		}
